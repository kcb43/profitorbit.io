      ...
      if (response.data?.immersive_products) {
        ...
        for (const product of response.data.immersive_products) {
          items.push({
            ...
            currency: 'USD',
            // FIX: Don't use serpapi_link as URL - it's a JSON API endpoint, not a merchant link!
            // Leave url empty if no direct link - frontend will fetch via immersive_product_page_token
            url: product.link && !product.link.includes('serpapi.com') ? product.link : null,
            product_link: product.link && !product.link.includes('serpapi.com') ? product.link : null,
